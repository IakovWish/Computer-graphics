...
void Fill_polygon(HDC hdc, const double ppts[M][N]) // заполнение полигона
{
    ...
    int count_1;
    for (count_1 = 0; count_1 < M; count_1++) // заполн€ем его 
    {
        pppts[count_1][X] = (ppts[count_1][X]);
        pppts[count_1][Y] = (ppts[count_1][Y]);
    }
    pppts[count_1][X] = (ppts[0][X]); // последн€€ точка равна нулевой точке
    pppts[count_1][Y] = (ppts[0][Y]);

    for (int count_2 = 0; count_2 < 800; count_2++) // выдел€ем пам€ть под 800 ребер
    {
