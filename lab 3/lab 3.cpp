...
BOOL Line(HDC hdc, int x1, int y1, int x2, int y2, int red, int green, int blue)
{
	HPEN hPen; //Объявляется кисть
	hPen = CreatePen(PS_SOLID, 1, RGB(red, green, blue)); //Создаётся объект
...
void paint(HDC hdc, double fig[M][N])
{
	int Xl = fig[0][0];
	int Xr = fig[0][0];
	int Yu = fig[0][1];
	...
	for (int Xtek = Xl, Ytek = Yu; Xtek != Xr && Ytek != Yd; Xtek = Xl, Ytek++)
	{
		int flag = 0;
		int Xst = Xr;
		int Xend = Xl;
		...
		{
			int verh = 0;
			for (int i = 0; i < M && verh == 0; i++)
			{
				if (Xtek == fig[i][0] && Ytek == fig[i][1])
				{
					verh = 1;
				}
			}
			...
			if (verh == 0)
			{
				int color = GetPixel(hdc, Xtek, Ytek);
				if (((GetRValue(color) != 255 && GetBValue(color) != 255 && GetGValue(color) != 255)) || GetBValue(color) != 255)
				{
					...
					else
					{
						...
						else
						{
							...
							if (Xend - Xst >= 2/* && fak == 0*/)
							{
								Line(hdc, Xst + 1, Ytek, Xend, Ytek, 0, 0, 255);
							}
						}
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT messg, WPARAM wParam, LPARAM lParam) // Функция окна
{
	...
	case WM_PAINT: // сообщение рисования
		...
		Line(hdc, hexagon[0][0], hexagon[0][1], hexagon[1][0], hexagon[1][1], 255, 0, 0); // 1
		Line(hdc, hexagon[1][0], hexagon[1][1], hexagon[2][0], hexagon[2][1], 255, 0, 0); // 2
		Line(hdc, hexagon[2][0], hexagon[2][1], hexagon[3][0], hexagon[3][1], 255, 0, 0); // 3
		Line(hdc, hexagon[3][0], hexagon[3][1], hexagon[4][0], hexagon[4][1], 255, 0, 0); // 4
		Line(hdc, hexagon[4][0], hexagon[4][1], hexagon[5][0], hexagon[5][1], 255, 0, 0); // 5
		Line(hdc, hexagon[5][0], hexagon[5][1], hexagon[0][0], hexagon[0][1], 255, 0, 0); // 6
